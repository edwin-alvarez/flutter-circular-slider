...
class CircularSliderSweepDecoration {
  ...
  double getRadius(double width, double height) =>
      min(width / 2, height / 2) - sliderStrokeWidth;

  CircularSliderSweepDecoration(
      {@required this.sliderStrokeWidth,
      this.gradient,
      this.color,
      this.useRoundedCap = true})
      : assert((gradient == null && color == null) ? false : true,
  ...
  paint(Canvas canvas, Size size, Offset center, double startAngle,
    ...
    canvas.drawArc(
        sweepRect, -pi / 2 + startAngle, sweepAngle, false, progress);
  }

  Paint _getPaint({double width, PaintingStyle style, Rect rect}) {
    var paint = Paint()
      ..strokeCap = useRoundedCap ? StrokeCap.round : StrokeCap.butt
...
class CircularSliderHandlerDecoration {
  ...
  CircularSliderHandlerDecoration(
      ...
      : assert((showhandlerOutter && shadow != null) ? false : true,
            'shadows does not draw well when using the defaul HandlerOutter, try using border instead'),
        assert((showhandlerOutter && icon != null) ? false : true,
            'handlerOutterRadius can not be use in convination with icon'),
        assert(
            (!showhandlerOutter ||
  ...
  void _drawShadow(Canvas canvas, Offset center) {
    ...
    Paint shadowPaint = Paint()
      ...
      ..maskFilter = MaskFilter.blur(BlurStyle.normal,
          Shadow.convertRadiusToSigma(shadow.blurRadius + shadow.spreadRadius));
...
class CircularSliderDeviderDecoration {
  ...
  CircularSliderDeviderDecoration(
      ...
      this.useRoundedCap = true})
      : assert(size > 0,
            "attribute [primaryDeviderSize] needs to be bigger than 0");
...
class CircularSliderDecoration {
  ...
  CircularSliderDecoration(
    this.sweepDecoration, {
  ...
  CircularSliderDecoration copyWith({
    ...
    return CircularSliderDecoration(
      ...
      secondDeviderDecoration:
          sndDeviderDecoration ?? this.secondDeviderDecoration,
