...
class MinMaxAngleValidator {
  ...
  MinMaxAngleValidator({
    ...
    this.endHandlerMaxValue,
            ...
            (iniHandlerMinValue == null ||
                (iniHandlerMinValue != null && iniHandlerMinValue >= 0)),
            "[iniHandlerMaxValue] must be equal or bigger than zero"),
        assert(
            (iniHandlerMaxValue == null ||
                (iniHandlerMaxValue != null && iniHandlerMaxValue >= 0)),
            "[iniHandlerMaxValue] must be equal or bigger than zero"),
        assert(
            (iniHandlerMinValue == null ||
                endHandlerMinValue == null ||
                (iniHandlerMinValue != null &&
                    endHandlerMinValue != null &&
                    iniHandlerMinValue < endHandlerMinValue)),
            "[endHandlerMinValue] value can not be bigger than [iniHandlerMinValue]"),
        assert(
            (iniHandlerMaxValue == null ||
                endHandlerMaxValue == null ||
                (iniHandlerMaxValue != null &&
                    endHandlerMaxValue != null &&
                    iniHandlerMaxValue < endHandlerMaxValue)),
            "[iniHandlerMaxValue] value can not be bigger than [endHandlerMaxValue]");

  bool validateIniHandler(
      int newValue,
      int initialEndHndLoc,
      int lastValidIniLoc,
      int tLaps,
      Function(int, int, int) onSelectionChange) {
    if (iniHandlerMaxValue != null && newValue > iniHandlerMaxValue) {
      onSelectionChange(
          lastValidIniLoc ?? iniHandlerMaxValue, initialEndHndLoc, tLaps);
      return false;
    }

    if (iniHandlerMinValue != null && newValue < iniHandlerMinValue) {
      onSelectionChange(
          lastValidIniLoc ?? iniHandlerMinValue, initialEndHndLoc, tLaps);
      return false;
    }
  ...
  bool validateEndHandler(
      int newValue,
      int initialIniHndLoc,
      int lastValidEndLoc,
      int tLaps,
      Function(int, int, int) onSelectionChange) {
    if (endHandlerMaxValue != null && newValue > endHandlerMaxValue) {
      onSelectionChange(
          initialIniHndLoc, lastValidEndLoc ?? endHandlerMaxValue, tLaps);
      return false;
    }

    if (endHandlerMinValue != null && newValue < endHandlerMinValue) {
      onSelectionChange(
          initialIniHndLoc, lastValidEndLoc ?? endHandlerMinValue, tLaps);
      return false;
    }
  ...
  bool validateSweepDrag(
      {@required int initialIniHndLoc,
      @required int lastValidIniLoc,
      @required int newIniValue,
      @required int initialEndHndLoc,
      @required int lastValidEndLoc,
      @required int newEndValue,
      @required int tLaps,
      @required Function(int, int, int) onSelectionChange}) {
    var status = validateIniHandler(newIniValue, initialEndHndLoc,
        lastValidIniLoc, tLaps, onSelectionChange);

    if (status)
      status = validateEndHandler(newEndValue, initialIniHndLoc,
          lastValidEndLoc, tLaps, onSelectionChange);
